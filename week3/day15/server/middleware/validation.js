...
const validateId = [
  param('id')
    .custom((value) => {
      if (isUuidLike(value) || isMongoIdLike(value)) return true;
      throw new Error('Invalid ID format: must be UUID (products/orders) or Mongo ObjectId (users)');
    }),
  handleValidationErrors
];
...
const validateMongoId = [
  param('id')
    .custom((value) => {
      if (isMongoIdLike(value)) return true;
      throw new Error('Invalid user ID format: must be a Mongo ObjectId');
    }),
  handleValidationErrors
];
