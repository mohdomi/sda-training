...
Validated: `node --check` OK on `babel.config.js`, `metro.config.js`, `index.js`; `package.json`/`app.json`/`tsconfig.json` parse as JSON; all 16 TS/TSX files have balanced braces/parens and every relative import resolves to an existing file. Not run (no RN toolchain / native SDKs here, `node_modules` intentionally not installed): `npm install`, `tsc --noEmit`, `react-native run-android` / `run-ios`.

Setup caveats before first run: `npm install`, then `npx react-native run-android|run-ios`; `process.env.API_BASE_URL` needs a `react-native-dotenv` (or equivalent) babel plugin to work on-device — otherwise the `http://localhost:3000/api/v1` fallback applies (use `10.0.2.2` instead of `localhost` from an Android emulator); `react-native-push-notification` and `react-native-vector-icons` need their documented native linking/font steps; point the API URL at the reachable day15–18 backend (`/auth/login`, `/auth/me`, `/users`, `/analytics`).
